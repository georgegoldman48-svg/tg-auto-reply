...
Система автоматически отвечает на входящие личные сообщения и сообщения в групповых чатах в Telegram, используя AI (Claude API или локальную модель). Поддерживает индивидуальные настройки для каждого контакта, триггеры для групповых чатов, синхронизацию с папкой Personal и лимиты ответов.
...
|-----------|--------|------|----------|
| **Worker** | v2.13 | `worker/auto_reply.py` | Обработчик автоответов (ЛС + чаты) |
...
#### `chat_triggers` - Триггеры для групповых чатов
| Поле | Тип | Описание |
|------|-----|----------|
| peer_id | BIGINT FK | Ссылка на peers (чат) |
| enabled | BOOLEAN | Триггеры активны |
| trigger_mention | BOOLEAN | Реагировать на @упоминание |
| trigger_reply | BOOLEAN | Реагировать на reply |
| trigger_keywords | BOOLEAN | Реагировать на ключевые слова |
| trigger_random | BOOLEAN | Рандомные сообщения |
| keywords | TEXT | Ключевые слова через запятую |
| random_interval_min | INT | Мин. интервал рандома (мин) |
| random_interval_max | INT | Макс. интервал рандома (мин) |
| cooldown_sec | INT | Кулдаун между ответами (сек) |
| daily_limit | INT | Лимит ответов в день |
...
| Чаты | Управление групповыми чатами |
| AI настройки | Выбор движка, промпт, температура |
| Синхронизация | Синхр. папки Personal |
| Новые контакты | Настройка для новых |

### Управление чатами
- **➕ Добавить** — добавить чат по ID или пересылкой сообщения
- **Триггеры**: @упоминание, Reply, Ключевые слова, Рандом
- **Настройки**: интервал рандома, кулдаун, дневной лимит
...
├── bots/
│   ├── __init__.py
│   └── admin_bot.py       # Admin Bot v3.5
├── worker/
│   ├── __init__.py
│   └── auto_reply.py      # Worker v2.13
├── collector/
│   ├── __init__.py
...
### v2.2 (2025-12-05)
- **Chat Triggers**: Полная поддержка групповых чатов
  - Триггеры: @упоминание, Reply, Ключевые слова, Рандом
  - Настраиваемый интервал и кулдаун
  - Дневной лимит ответов в чате
  - Фоновая задача для рандомных сообщений
- **Admin Bot v3.5**:
  - Меню "Чаты" с управлением групповыми чатами
  - Кнопка "➕ Добавить" с инструкцией
  - Добавление чата по ID (из @getmyid_bot) или пересылкой
  - Настройка триггеров через inline-кнопки
- **Worker v2.13**:
  - Обработка сообщений из групповых чатов
  - Проверка триггеров (mention/reply/keywords)
  - Фоновая задача random_chat_task
